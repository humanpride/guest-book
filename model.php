...
class DbConnect
{
    ...
    public function __construct(string $dbHost, string $dbUser, string $dbPass, string $dbName, string $charset)
    {
    ...
    public function save(string $username, string $email, string $homepage, string $text, array $tags, string $createdAt)
    {
    ...
    public function printData(string $statement, int $total, int $pageNum, int $configNumRows, bool $printForSearch)
    {
   	    ...
   	    if ($stmt->num_rows == 0) {
   	        if ($printForSearch) {
                ...
                echo "<div class='alert alert-light' role='alert'>
                        Записей в книге нет. Вы можете <a href='#' onclick='return fakeData()'>создать фиктивные записи</a>.
                    </div>";
            }
   	    } else {
   	        echo "<form class='form-inline justify-content-end'>
